...
function calculate() {
    ...
    //top Horiz Tape
    frameTapeHoriz[0].style.height = fBase.value * scale.value - brdThick.value*scale.value +'px'; 
    frameTapeHoriz[0].style.width = (fWidth.value * scale.value) - (2*(fBase.value*scale.value))+'px';
    frameTapeHoriz[0].style.margin = fBase.value * scale.value;
    frameTapeHoriz[0].innerText = `${fBase.value - brdThick.value}"`;
    //Top Horiz Return
    frameReturnHoriz[0].style.height = fDepth.value * scale.value - brdThick.value*scale.value +'px'; 
    frameReturnHoriz[0].style.width = (fWidth.value * scale.value) - (2*(fBase.value*scale.value))+'px';
    ...
    frameBaseHoriz[0].style.borderRight = fBase.value*scale.value +"px solid transparent";
    frameBaseHoriz[0].style.borderLeft = fBase.value * scale.value +"px solid transparent";
    frameBaseHoriz[0].style.borderBottom = fBase.value * scale.value +"px solid var(--secondary-color)";
    ...
    frameBase[0].style.borderRight = fBase.value*scale.value +"px solid var(--secondary-color)";
    frameBase[0].style.borderTop = fBase.value * scale.value +"px solid transparent";
    frameBase[0].style.borderBottom = fBase.value * scale.value +"px solid transparent";
    baseText[2].innerText = `${fBase.value}"`;
    //Right Base
    frameBase[1].style.height = fHeight.value * scale.value +'px'; 
    frameBase[1].style.width = 0;
    frameBase[1].style.borderLeft = fBase.value*scale.value +"px solid var(--secondary-color)";
    frameBase[1].style.borderTop = fBase.value * scale.value +"px solid transparent";
    frameBase[1].style.borderBottom = fBase.value * scale.value +"px solid transparent";
    for(let e=0;e<frameDepth.length;e++) {
    baseText[e].innerText = `${fBase.value}"`;
    }
    //Right and Left Return
    for(let x=0;x<frameReturn.length;x++) {
        frameReturn[x].style.height = (fHeight.value * scale.value) - (2*(fBase.value*scale.value))+'px'; 
        frameReturn[x].style.width = fDepth.value * scale.value - brdThick.value*scale.value +'px';
    ...
    for(let x=0;x<frameTape.length;x++) {
        frameTape[x].style.height = (fHeight.value * scale.value) - (2*(fBase.value*scale.value))+'px'; 
        frameTape[x].style.width = fBase.value * scale.value - brdThick.value*scale.value +'px';
    ...
    frameBaseHorizBtm[0].style.borderRight = fBase.value*scale.value +"px solid transparent";
    frameBaseHorizBtm[0].style.borderLeft = fBase.value * scale.value +"px solid transparent";
    frameBaseHorizBtm[0].style.borderTop = fBase.value * scale.value +"px solid var(--secondary-color)";
    ...
    //Bottom Horiz Return
    frameReturnHorizBtm[0].style.height = fDepth.value * scale.value - brdThick.value*scale.value +'px'; 
    frameReturnHorizBtm[0].style.width = (fWidth.value * scale.value) - (2*(fBase.value*scale.value))+'px';
    frameReturnHorizBtm[0].style.margin = fBase.value * scale.value;
    frameReturnHorizBtm[0].innerText = `${fDepth.value - brdThick.value}"`
    //Bottom Horiz Tape
    frameTapeHorizBtm[0].style.height = fBase.value * scale.value - brdThick.value*scale.value +'px'; 
    frameTapeHorizBtm[0].style.width = (fWidth.value * scale.value) - (2*(fBase.value*scale.value))+'px';
...
function boardSize() {
 let width = ((fBase.value*4)+(fDepth.value*4)-(brdThick.value*4)+parseInt(fWidth.value)) * scale.value;
 let height = ((fBase.value*4)+(fDepth.value*4)-(brdThick.value*4)+parseInt(fHeight.value)+.5) * scale.value;

 foldAttrib.style.height = height+'px';
 foldAttrib.style.width = width+'px';
 dims.innerHTML = `The flat size of board needed is ${width/scale.value}" w x ${height/scale.value}" h`
}
